...
export default function AdminDashboard({ activeTab: initialTab = 'results' }: { activeTab?: 'results' | 'voters' | 'candidates' | 'settings' }) {
  ...
  const [addingDefaultCandidates, setAddingDefaultCandidates] = useState(false);
  ...
  const handleAddDefaultCandidates = async () => {
    setAddingDefaultCandidates(true);
    setCandidateMessage('');

    try {
      const res = await fetch('/api/admin/add-default-candidates', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      ...
      if (data.success) {
        if (data.added && data.added.length > 0) {
          ...
          setCandidateMessage(`✓ ${data.message || 'Default candidates already exist'}`);
        }
        fetchCandidates();
      } else {
        setCandidateMessage(`✗ Error: ${data.error || 'Failed to add default candidates'}`);
      }
    } catch (error) {
      setCandidateMessage(`✗ Error: ${error instanceof Error ? error.message : 'Failed to add default candidates'}`);
    } finally {
      setAddingDefaultCandidates(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
          ...
          {activeTab === 'candidates' && (
            <div className="space-y-6">
              {/* Add Default Candidates Button */}
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1" style={{ fontFamily: 'var(--font-anton), sans-serif' }}>
                      Quick Setup
                    </h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400" style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}>
                      Add default candidates (Candidate 1 & Candidate 2) to get started quickly
                    </p>
                  </div>
                  <button
                    onClick={handleAddDefaultCandidates}
                    disabled={addingDefaultCandidates}
                    ...
                    style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}
                  >
                    {addingDefaultCandidates ? 'Adding...' : 'Add Default Candidates'}
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/50 rounded-lg" style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}>
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="max-w-2xl">
                  <p className="text-gray-600 dark:text-gray-300 mb-4" style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}>
                  ...
                  <div className="space-y-4">
                    ...
                    <button
                      ...
                      className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-all duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                      style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}
                    >
                    ...
                    {csvMessage && (
                      <div className={`mt-4 p-4 rounded-lg whitespace-pre-wrap ${
                        ...
                        <p className="text-sm" style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}>{csvMessage}</p>
                      </div>
                    )}
                  </div>
                  <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    <p className="text-sm text-blue-800 dark:text-blue-300 font-medium mb-2" style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}>CSV Format Example:</p>
                ...
                <div className="max-w-md">
                  ...
                  {syncMessage && (
                    <div className={`mt-4 p-4 rounded-lg ${
                      ...
                      <p className="text-sm" style={{ fontFamily: 'var(--font-alexandria), sans-serif' }}>{syncMessage}</p>
                    </div>
                  )}
                </div>
              </div>
